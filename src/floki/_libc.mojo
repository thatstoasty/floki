...
struct SocketType:
    var value: Int32
    alias SOCK_STREAM = Self(1)
    alias SOCK_DGRAM = Self(2)
    alias SOCK_RAW = Self(3)
    alias SOCK_RDM = Self(4)
    alias SOCK_SEQPACKET = Self(5)
    alias SOCK_DCCP = Self(6)
    alias SOCK_PACKET = Self(10)
    alias SOCK_CLOEXEC = Self(O_CLOEXEC)
    alias SOCK_NONBLOCK = Self(O_NONBLOCK)

    fn __init__(out self, value: Int):
        """Initialize a SocketType with a specific value."""
        self.value = value
...
@fieldwise_init
struct in6_addr(Movable):
    var s6_addr: InlineArray[c_char, 16]
...
struct sockaddr:
    ...
    fn __init__(out self, family: sa_family_t = 0, data: InlineArray[c_char, 14] = InlineArray[c_char, 14]()):
        self.sa_family = family
        self.sa_data = data


@fieldwise_init
struct sockaddr_in(Movable):
    ...
    fn __init__(out self, address_family: Int, port: UInt16, binary_ip: UInt32):
        ...
        self.sin_zero = InlineArray[c_char, 8](0, 0, 0, 0, 0, 0, 0, 0)
